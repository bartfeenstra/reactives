...
from reactives import ReactorController, Reactor, Reactive, ReactorDefinition, UnsupportedReactive, reactive, \
    isreactive, ReactiveDict, ReactiveList, _InstanceReactorController, Scope
...
class ReactiveTestCase(TestCase):
    ...
    def assert_reactor_called(self, sut: Union[Reactive, ReactorController, None] = None) -> Union[Reactor, None]:
        def reactor() -> None:
            if reactor.called:
                self.fail(
                    'Failed asserting that a reactor (%s) was called only once.' % reactor)
            reactor.called = True
        reactor.called = False
        yield from self._assert_reactor(reactor, sut)
        self.assertTrue(
            reactor.called, 'Failed asserting that a reactor (%s) was called, but it was actually never called at all.' % reactor)

    @contextmanager
    def assert_not_reactor_called(self, sut: Union[Reactive, ReactorController, None] = None) -> Union[Reactor, None]:
        def reactor() -> None:
            self.fail(
                'Failed asserting that a reactor (%s) was not called.' % reactor)
        yield from self._assert_reactor(reactor, sut)
...
class ReactivePropertyTest(ReactiveTestCase):
    ...
    def test_fdel(self) -> None:
        ...
        subject.react.getattr('subject').react.react_weakref(
            self.assert_not_reactor_called())
        dependency.react.trigger()
